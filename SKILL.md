...
3. For `./starter.sh build`, request one combined elevated sandbox approval for that exact command. The approval question must state both that it performs the cloud operation described above and that it needs outbound network access to OCI using the user's locally configured credentials. Treat approval of this single escalation as the user's explicit permission for the build; do not ask a separate, sequential cloud-operation confirmation.
